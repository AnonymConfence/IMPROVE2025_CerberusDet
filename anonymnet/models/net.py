...
from anonymnet.models.common import Bottleneck  # noqa: F401
from anonymnet.models.common import (  # noqa: F401
    C2,
    C3,
    C3SPP,
    C3TR,
...
from anonymnet.models.yolo import Detect, Model, get_next_layer_from_cfg, initialize_weights
from anonymnet.utils.torch_utils import de_parallel, fuse_conv_and_bn
...
class Controller:
    """
    OurAnonymModel's block controller. Stores information about its index in the
    ...
    Attributes:
      index:             the index of this block in the OurAnonymModel.blocks
      execution_chain:   indices of blocks to be executed prior to this
      parent_index:      index (in OurAnonymModel.blocks) of the parent block
      children_indices:  indices (in OurAnonymModel.blocks) of the childrens
...
class OurAnonymModel(nn.Module):
    ...
    def parse_neck(self, prev_model, cfg, ch, nc):
        ...
        for i, layer in enumerate(cfg["neck"], start=1):
            ...
            for x in f:
                if x != -1 and x >= len(cfg["backbone"]):
                    ...
                    raise ValueError("Input for first model block must be defined")
        ...
        if self.verbose:
            LOGGER.info(f"Neck parameters: {n_p} layers: {n_l}. Sharable modules: {n_s}")
    ...
    def parse_heads(self, prev_model, cfg, task_ids, ch, inplace, layer_ind_map, nc):
        ...
        for task_id in task_ids:
            ...
            for i, layer in enumerate(cfg["head"]):
                ...
                np, nl = sum(x.numel() for x in m_.parameters()), len(list(m_.modules()))
                n_p, n_l = n_p + np, n_l + nl
            ...
            if self.verbose:
                LOGGER.info(f"{task_id} head parameters: {n_p} layers: {n_l}")
    ...
    def add_block(self, module):
        """
        Registers a new OurAnonymModel block, automatically adds it to the
    ...
    def add_head(self, module, task_id):
        """
        Registers a new OurAnonymModel block as a "Head". Same as the method
    ...
    def split(self, index, branching_scheme, device, next_configs):
        """
        ...
        for sc in next_configs:  # schedule e.g.: [[4, [[16, 17], [15]]], [12, [[17], [16]]]]
    ...
    def sequential_split(self, schedule, device):
        """
        Sequentially splits a model's block into several blocks, according to the
        each `branching_scheme` in `schedule`
        """
        schedule_head_ids = [list(itertools.chain(*conf[-1])) for conf in schedule]
        ...
        ), f"Invalid anonymnet config {schedule}"

        for i in range(len(schedule)):
            branching_scheme = schedule[i]  # e.g. branching_scheme: [2, [[15], [16, 17]]]
            next_configs = schedule[i + 1 :]
            ...
            for ii, next_branching_scheme in enumerate(next_configs):
                ...
                schedule[i + 1 + ii][0] = mapped_ind[0]

            # print(shared_model.info())
            # print("Next configs: ", schedule[i+1:])

    def peel(self, task_ids, device=None):
        """
        Peels off a task-specific subnetwork. Results of peel('task_a'):
        ...
        Args:
          ...
          device:    a device to spawn new peeled model on

        Returns:
          peeled_model: A new model that is only related to secified tasks.
        ...
        new_model.controllers = list()
        new_model.heads = dict()
        new_model.neck_head_save = list()
        ...
        for index in execution_order:
            ...
            if controller.task_id is not None:
                new_model.add_head(new_block, controller.task_id)
            else:
                new_model.add_block(new_block)

        for index in execution_order:
            new_index = index_map[index]
            controller = self.controllers[index]
            new_controller = new_model.controllers[new_index]

            if index in self.neck_head_save:
                new_model.neck_head_save.append(new_index)
            ...
            if parent_index is not None and isinstance(parent_index, int):
                ...
                new_parent = new_model.controllers[new_parent_index]
                new_controller.stack_on(new_parent)
            elif isinstance(parent_index, list):
                for pi in parent_index:
                    new_parent_index = index_map[pi]
                    new_parent = new_model.controllers[new_parent_index]
                    new_controller.add_parent(new_parent, new_model.controllers)

        new_model.build()
        return new_model, index_map
    ...
    @staticmethod
    def unfreeze_shared_layers(model):
        model = de_parallel(model)
