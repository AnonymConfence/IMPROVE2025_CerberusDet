...
from anonymnet.utils.checks import check_file
from anonymnet.utils.ckpt_utils import dict_to_anonymnet, intersect_dicts
from anonymnet.utils.general import check_dataset, colorstr, labels_to_class_weights, strip_optimizer
from anonymnet.utils.mlflow_logging import MLFlowLogger
from anonymnet.utils.plots import plot_images
from anonymnet.utils.torch_utils import (
    ModelEMA,
    de_parallel,
    get_hyperparameter,
    is_parallel,
...
class ModelManager:
    ...
    def from_ckpt(self, ckpt, model, exclude=[]):
        ...
        if "blocks." in list(model.state_dict().keys())[0] and "blocks." not in list(state_dict.keys())[0]:
            # if loading weights from yolov5 to anonymnet
            state_dict = dict_to_anonymnet(state_dict, model)  # intersect
            loaded = True
    ...
    def load_model(self, cfg, device, verbose=True):
        ...
        if model.yaml.get("anonymnet") and len(model.yaml["anonymnet"]):
            model_schedule = model.yaml["anonymnet"]
            # model_schedule is e.g. [[0, [[17], [15, 16]]], [4, [[15], [16]]]]
            if self.rank in [-1, 0] and self.loggers["mlflow"]:
                self.loggers["mlflow"].log_params({"anonymnet": model_schedule})
            model.sequential_split(deepcopy(model_schedule), device)
        ...
        if pretrained and not loaded:
            # if loading weights from pretrained anonymnet
            state_dict = intersect_dicts(state_dict, model.state_dict(), exclude=exclude)  # intersect
            model.load_state_dict(state_dict, strict=False)  # load
            LOGGER.info(
