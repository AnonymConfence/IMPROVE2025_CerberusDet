...
import torch
import yaml
from anonymnet.data.dataloaders import create_dataloader
...
from anonymnet.utils.general import (
    box_iou,
    check_dataset,
    check_img_size,
    colorstr,
...
from anonymnet.utils.metrics import ConfusionMatrix, DetMetrics
from anonymnet.utils.mlflow_logging import MLFlowLogger
from anonymnet.utils.plots import output_to_target, plot_images
from anonymnet.utils.torch_utils import Profile, model_info, select_device
...
def get_stats(stats, metrics: DetMetrics, nc: int, task_id: str):
    stats = [torch.cat(x, 0).cpu().numpy() for x in zip(*stats)]  # to numpy
    if len(stats) and stats[0].any():
        metrics.process(*stats, task_id=task_id)
...
def run(
    ...
    print(f"nms conf: {conf_thres}, nms iou: {iou_thres}")
    ...
    for task_i, (task_id, nc, dataloader) in enumerate(zip(task_ids, tasks_nc, dataloaders)):
        ...
        for batch_i, batch in enumerate(tqdm(dataloader, desc=s)):
            ...
            # inference
            with dt[1]:
                if inference_all and isinstance(model, OurAnonymModel):
                    # inference all tasks
        ...
        # Compute statistics
        stats_dict, nt_per_class = get_stats(stats, metrics, nc, task_id)
