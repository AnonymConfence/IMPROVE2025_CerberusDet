...
from anonymnet.trainers.base_trainer import BaseTrainer
from anonymnet.utils.general import colorstr, one_cycle
from anonymnet.utils.plots import plot_lr_scheduler
from anonymnet.utils.torch_utils import de_parallel, get_hyperparameter
...
class Averaging(BaseTrainer):
    ...
    def train_epoch(self, model, ema, epoch, local_rank, world_size):
        ...
        if hasattr(de_parallel(model), "heads"):  # OurAnonymNet
            if epoch < self.opt.freeze_shared_till_epoch:
                OurAnonymModel.freeze_shared_layers(model)
            elif 0 < self.opt.freeze_shared_till_epoch == epoch:
                OurAnonymModel.unfreeze_shared_layers(model)
