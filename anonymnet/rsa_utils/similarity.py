...
def get_similarity_from_rdms(x, y, dist, feature_norm, debiased=True, centered=True, flattened=True, h=16, w=16, c=8):
    """
    Parameters
    ----------
    ...
    if feature_norm == "None":
        ...
        if flattened:
            x = np.reshape(x, (-1, h, w, c))  # reshaping features back to n x h x w x c format from flattened features
            y = np.reshape(y, (-1, h, w, c))  # reshaping features back to n x h x w x c format from flattened features
    ...
    rdm_x = rdm(x, dist)
    rdm_y = rdm(y, dist)
    # print(x.shape, y.shape, rdm_x.shape, rdm_y.shape)
    return cka(rdm_x, rdm_y, debiased=debiased, centered=centered)


def get_similarity(x, y, kernel, feature_norm, debiased=True, centered=True, flattened=True, h=16, w=16, c=8):
    """
    Parameters
    ----------
    x : numpy matrix with dimensions n x p
...
def rdm(activations_value, dist):
    """
    For each pair of conditions (images),
    compute a dissimilarity score 1−ρ, where ρ is the Pearson’s correlation coefficient
...
def cka(gram_x, gram_y, debiased=False, centered=True):
    ...
    It uses HSIC (Hilbert-Schmidt Independence Criterion)
    Replacement for Spearman's correlation.
    ...
    Returns:
      The value of CKA between X and Y (similarity score).
      P.S. Function from Kornblith et al., ICML 2019
    """
    if centered:
